...
RUTA_ARCHIVO = r'C:\Users\Karina\Desktop\Tesis\Paneles2\Panelcaso3.csv'
...
def imprimir_control_unico(antigeno, df, resultados):
    ...
    print(f"[{estado}] Anti-{antigeno}: {n_homo_pos} células reactivas homocigotas (dosis doble) y {n_neg_no_reactivas} células negativas no reactivas.")
...
def imprimir_control_mezcla(antig_1, antig_2, df, resultados_paciente, col_ahg, col_enz, usar_enz):
    ...
    if usar_enz and (destruido_1 != destruido_2):
        ...
        print(f"[{est_res}] Anti-{resistente} (Evaluado en ENZ): {n_pos_res} células reactivas homocigotas y {n_neg_res} células negativas no reactivas.")
        print(f"[{est_dest}] Anti-{destruido} (Evaluado en AHG puro): {n_pos_dest} células reactivas homocigotas puras y {n_neg_dest} células negativas puras no reactivas.")
        ...
        print(f"[{estado_1}] Anti-{antig_1}: {n_homo_pos_1} células reactivas homocigotas puras y {n_neg_no_reactivas_1} células negativas puras no reactivas.")
        print(f"[{estado_2}] Anti-{antig_2}: {n_homo_pos_2} células reactivas homocigotas puras y {n_neg_no_reactivas_2} células negativas puras no reactivas.")
...
if len(coincidencias_completas) == 1:
    ...
    print(f"\nResultado: Anti-{antig_confirmar_u}")
        ...
        if evaluaciones_mezclas and evaluaciones_mezclas[0]['score'] > 0:
            ...
            if candidatos_validos_unicos:
                ...
                if sospechosos_alta:
                    ...
                    print(f"\n[SOPORTE ALTA FRECUENCIA] Detectado patrón compatible con anticuerpo de alta frecuencia.")
                else:
                    print("\nResultado: No se pudo determinar un anticuerpo o mezcla probable.")
...
if confirmar_mezcla and len(confirmar_mezcla) == 2:
    ...
    if not mezcla_es_coherente:
        ...
        print(f"\n[FILTRO DE COHERENCIA] La mezcla propuesta (Anti-{m1} + Anti-{m2}) presenta un patrón de reacción plano (sin variación de dosis).")
        print("-> Re-evaluando sospecha de Anticuerpo contra Antígeno de Alta Frecuencia...")
        
        sospechosos_alta = evaluar_alta_frecuencia(datos, resultados_paciente, COLUMNA_PACIENTE)
        if sospechosos_alta:
            antig_confirmar_u = sospechosos_alta[0]
            confirmar_mezcla = None  # Cancelar la mezcla
            print(f"\nResultado definitivo: Anti-{antig_confirmar_u}")
        else:
            # Si no se encontró un alta frecuencia claro, se mantiene la mezcla con una advertencia de patrón plano
            print(f"\nResultado (Mezcla más probable con advertencia): Anti-{m1} + Anti-{m2} (Reacciones inusualmente planas)")
    else:
        print(f"\nResultado (Mezcla más probable): Anti-{m1} + Anti-{m2}")
...
if antig_confirmar_u:
    ...
    print("\nControl de confirmación 3+3 (Mezcla múltiple):")
    for susp in confirmar_mezcla:
        ...
        estado = "Cumple" if cumple else "No cumple"
        print(f"[{estado}] Anti-{susp}: {n_pos} células reactivas y {n_neg_u} células negativas puras no reactivas.")